...
void CvDetection::ProcessState() {
    switch (m_state_) {
        ...
        case ST_FINE_DETECTION: {
            ...
            //0.算出来的点
            Eigen::Vector3f input(location.center_point[0], location.center_point[1], location.center_point[2],1);
            //1.输出的点
            ...
            std::vector<float> move_postition{output(0),outpu(1),output(2)+50,xarm_state[3], xarm_state[4], xarm_state[5]};
            ArmMove(move_postition);
