...
Objection::Objection(cv::Rect Box, string name){
    ...
    for(int i=Aera_Objection_R.x;i<Aera_Objection_R.x+Aera_Objection_R.width;i++){
        for(int j=Aera_Objection_R.y;j<Aera_Objection_R.y+Aera_Objection_R.height;j++){
    ...
    //4.将plane cloud转换为pcl::PointCloud<pcl::PointXYZRGB> 发布出去
...
//--------------------------RANSAC拟合平面--------------------------
	...
	pcl::copyPointCloud<pcl::PointXYZ>(*raw_cloud, inliers, *cloud_plane);
	//----------------------------输出模型参数---------------------------
    ...
    //     std::cerr << idx << "    " << raw_cloud->points[idx].x << " "
    //                             << raw_cloud->points[idx].y << " "
    //                             << raw_cloud->points[idx].z << std::endl;
