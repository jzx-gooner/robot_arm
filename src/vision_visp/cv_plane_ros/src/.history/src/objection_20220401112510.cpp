...
Objection::Objection(cv::Rect Box, string name){
    ...
    Eigen::Vector3f center_point{centroid[0],centroid[1],centroid[2]};
    target_point  = center_point +300*normal;//30cm处
    std::cout<<"目标点位置 ： "<<target_point[0]<<" , "<<target_point[1]<<" , "<<target_point[2]<<std::endl;
    ...
    plane_pose = {target_point[0], target_point[1], target_point[2], roll_rad, 0, pitch_rad};
