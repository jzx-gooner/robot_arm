    ...
    else
    {
        ...
        for (auto &obj : det_objs)
        {
            ...
            {   
                ...
                cv::Rect boundingbox(20, 20, 100, 100);
                cv::rectangle(img, cv::Point(20, 20), cv::Point(120, 120), cv::Scalar(255, 0, 0), -1);
        ...
        {
            cv::imwrite("/home/jzx/AAA_result/detection_result.jpg", img);
            ofstream OutFile("/home/jzx/AAA_result/out.txt");
            // OutFile << "The detection result" << endl; //写入文本";
            for (auto obj2d : objetsin2Dimage)
            {
                OutFile << obj2d.center_point[0] << "," << obj2d.center_point[1] << "," << obj2d.center_point[2] << std::endl;
            }
            OutFile.close();
        ...
        // objetsin3Dpointcloud.pointcloud.header.frame_id = "camera_link";
...
bool CvDetection::saveServerClient(cv_plane::serverSaveDetectionResult::Request &req, cv_plane::serverSaveDetectionResult::Response &res)
{
    res.result = "OK";
