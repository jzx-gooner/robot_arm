...
std::array<int, 3> Position_Transform::Get_XYZ() {
    ...
    for(int i=start_x;i<=end_x;i++){
        for(int j=start_y;j<=start_y+stride*2;j++){
            if(Depthmat.at<uint16_t>(j,i)>200 & Depthmat.at<uint16_t>(j,i)<6000 ){
                sum_count++;
...
Eigen::Vector3f Position_Transform::Get_ROBOT_TOOL_XYZ() {
    ...
    //1.工具坐标系到基坐标的的变换矩阵
        Eigen::Vector3d ea(0, -2.2995266333927507e-16, -3.1415927410125732);  //0 1 2 对应 z y x
        Eigen::Matrix3d R_2;
        R_2 = Eigen::AngleAxisd(ea[0], Eigen::Vector3d::UnitZ()) *
                        Eigen::AngleAxisd(ea[1], Eigen::Vector3d::UnitY()) *
                        Eigen::AngleAxisd(ea[2], Eigen::Vector3d::UnitX());
        // cout<<"R_2:"<<R_2<<endl;
        Eigen::Vector3d T_2 = Eigen::Vector3d(506, -35, 413);
