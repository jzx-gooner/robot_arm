...
Objection::Objection(cv::Rect Box, string name){
    ...
    center_ss << "("<<static_cast<int>(center_location.at(0))<<","<<static_cast<int>(center_location.at(1))<<","<<static_cast<int>(center_location.at(2)) <<")";
    
    std::array<int, 3> grasp_world = Position_Transform::Get_ROBOT_XYZ();
    ostringstream grasp_ss;
    grasp_ss<<""<<static_cast<int>(grasp_world.at(0))<<","<<static_cast<int>(grasp_world.at(1))<<","<<static_cast<int>(grasp_world.at(2))<<"";
    putText(color_mat, center_ss.str(), Point(center_x, center_y-10), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 255), 1);
