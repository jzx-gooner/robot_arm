...
Eigen::Vector3f Position_Transform::Get_ROBOT_TOOL_XYZ() {
    ...
    //相机坐标系到工具坐标系的变换矩阵
    Eigen::Matrix4d Trans_CameraToTool;
    Trans_CameraToTool.setIdentity();   // Set to Identity to make bottom row of Matrix 0,0,0,1
    Trans_CameraToTool.block<3,3>(0,0) = R;
    Trans_CameraToTool.block<3,1>(0,3) = T;
    
    //1.工具坐标系到基坐标的的变换矩阵
    
    Eigen::Quaterniond q_2(0.485131762248, -0.495642165723 ,-0.522309662329 ,-0.496163917926);
    q_2.normalize();
    Eigen::Matrix3d R_2 = q_2.toRotationMatrix();
    //Eigen::Matrix3d R_23=R_2.transpose();

    Eigen::Vector3d T_2 = Eigen::Vector3d(0.0344786754531, 0.057878017888, -0.0228528460852);
    Eigen::Matrix4d Trans_CamToTool; // Your Transformation Matrix
    Trans_CamToTool.setIdentity();   // Set to Identity to make bottom row of Matrix 0,0,0,1
    Trans_CamToTool.block<3,3>(0,0) = R_2;
    Trans_CamToTool.block<3,1>(0,3) = T_2;

    //2.相机坐标系到基坐标系

    Eigen::Matrix<double,4,4> matrix_ObjToBase;
    matrix_ObjToBase=Trans_ToolToBase*Trans_CamTotool;
