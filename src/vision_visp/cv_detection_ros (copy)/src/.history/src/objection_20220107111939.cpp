...
Objection::Objection(cv::Rect Box, string name){
    ...
    // Arr_Box.at((Box.y)*Stride + (Box.x)) = Depthmat.at<uint16_t>(i, j);
...
void Objection::DealRect() {
    ...
    cout<<"depth_mat : " <<Depthmat.size()<<endl;
    if (Depthmat.size()==cv::Size(0,0)) {
        cout<<"Depthmat is null"<<endl;
        return;
    }
    cv::Mat Object_Area_Depth = Depthmat(Area_Objection_D);//截取目标范围的深度图
