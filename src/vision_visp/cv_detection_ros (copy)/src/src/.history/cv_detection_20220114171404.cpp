...
void CvDetection::infer(cv::Mat &img)
{
    ...
    else
    {
        ...
        ofstream OutFile("/home/jzx/out.txt"); //利用构造函数创建txt文本，并且打开该文本
        ...
        for (auto &obj : det_objs)
        {
            if (true) //手机是67 obj.class_label == 67 15是猫
            {
                ...
                cv::circle(img, center, 15, cv::Scalar(b, g, r), -1);
                // 坐标系变换
                auto temp = Objection(boundingbox, name);
                OutFile<<temp.center_point.at(0)<<temp.center_point.at(1)<<temp.center_point.at(2)<<temp.center_point.at(3)<<std::endl;
                cout<<"temp centerpoint:"<<temp.center_point.at(0)<<endl;
