...
private:
    ...
    Eigen::Vector3f Depth_cam,Color_cam;//相机坐标系下的坐标
    Eigen::Vector3f RGB_Pix_position;//RGB像素坐标
...
public:
    ...
    Eigen::Vector3f Get_Rough_XYZ();//返回相对于机械臂底座的3D位置
    Eigen::Vector3f Get_Fine_XYZ();//返回相对于机械臂底座的3D位置
    ~Position_Transform();
};
