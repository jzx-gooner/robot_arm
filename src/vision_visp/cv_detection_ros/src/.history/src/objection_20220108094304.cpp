...
Objection::Objection(cv::Rect Box, string name){
    ...
    cout<<"111111111111 center_x:"<<center_x<<"center_y:"<<center_y<<endl;
    Position_Transform PT(array<int,2>{center_x,center_y}, true);
    std::array<int, 3> center_location=PT.Get_XYZ();//转换
    ostringstream ss;
    ss << "("<<static_cast<int>(center_location.at(0))<<","<<static_cast<int>(center_location.at(1))<<","<<static_cast<int>(center_location.at(2)) <<")";
    putText(color_mat, ss.str(), cv::Point(center_x, center_y), 0, 0.3, cv::Scalar(0, 255, 0));

    //3.转换彩色图的坐标到深度图,得到深度图的目标框
    // Position_Transform Start_point_D(array<int,2>{Aera_Objection_R.x,Aera_Objection_R.y}, true);
    // array<int,2> Start_pix=Start_point_D.Report_Depth_Pix();//转换
