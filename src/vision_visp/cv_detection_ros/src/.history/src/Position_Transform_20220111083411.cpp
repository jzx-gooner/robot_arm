...
//返回相机坐标系下的位置
std::array<int, 3> Position_Transform::Get_ROBOT_XYZ() {
    ...
    //初始化顺序，wxyz
    Quterniond q((qw,qx,qy,qz);
    Eigen::Matrix3d R = q.toRotationMatrix();

    //四元数转成旋转矩阵
	float w = hand_eye_quaternion.at<float>(0);
	float x = hand_eye_quaternion.at<float>(1);
	float y = hand_eye_quaternion.at<float>(2);
	float z = hand_eye_quaternion.at<float>(3);
	...
	Mat R = cv::Mat(4, 4, CV_32FC1, ret).clone();
    ...
    //相机坐标系下的位置
    Mat camra_frame_pos_4x1 = (cv::Mat_<float>(4, 1) << PCL_Position.at(0),
                            PCL_Position.at(1),
                            PCL_Position.at(2),
                            1.0);
    //机械臂工具坐标系下的位置
    ...
    std::array<int, 3> world_frame_pos_3x1 = { gripper_frame_pos_4x1.at<int>(0, 0),
								   gripper_frame_pos_4x1.at<int>(0, 1),
								   gripper_frame_pos_4x1.at<int>(0, 2)};
    return world_frame_pos_3x1;
}
